...
const QUESTION_PRIMARY_MODEL = process.env.OPENROUTER_QUESTION_MODEL || process.env.OPENROUTER_MODEL || DEFAULT_MODEL;
const QUESTION_FALLBACK_MODELS = (process.env.OPENROUTER_QUESTION_FALLBACK_MODELS || process.env.OPENROUTER_MODEL_FALLBACKS || "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
...
`;
    ...
    try {
        const { result: response, model: selectedModel } = await executeWithAiResilience({
            ...
            maxRetries: 2,
            execute: async (model) => axios.post(OPENROUTER_ENDPOINT, {
                model,
                messages: [
