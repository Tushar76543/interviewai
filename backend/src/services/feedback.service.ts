...
const sanitizeList = (value: unknown, maxItems: number, itemMaxLength: number) => {
...
const matchesExpectedPoint = (expectedPoint: string, normalizedAnswer: string) => {
...
const parseFeedback = (raw: string): FeedbackPayload | null => {
  try {
    ...
    const sanitizedFeedback: FeedbackBreakdown = {
      technical,
      clarity,
      completeness,
      overall:
